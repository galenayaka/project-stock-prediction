"""
ML Predictor: loads model and generates stock price predictions
with confidence scores and feature importance.

Supports training on historical price data from yfinance
as well as inference from financial-statement features.
"""
...
# Required feature columns expected by the model (in order)
...
# Technical features used by the price-forecasting model
...
class StockPredictor:
    ...
    Two modes:
    1. Financial-statement mode  – predict from fundamental features (pe_ratio, eps, …)
    2. Technical / price-forecast mode – predict future close from OHLCV + derived features

    Call ``train_on_price_history()`` to fit the models on real yfinance data.
    """
    ...
    def __init__(self) -> None:
        ...
        # Evaluation metrics from last training run
    ...
    def load_model(self) -> None:
        """
        Initialise untrained model objects.

        To restore a previously trained model, call ``train_on_price_history()``
        or load serialised artefacts with joblib.
        """
    ...
    def train_on_price_history(
        ...
        """
        Train the ensemble on historical OHLCV data (from YFinanceFetcher).

        Expects a DataFrame with the columns returned by
        ``YFinanceFetcher.get_training_data()``, including:
        - Feature columns: open, high, low, close, volume, returns_5d,
          returns_20d, volatility_20d, volume_ratio, sma_20, sma_50,
          price_to_sma20
        - Target column: target_close

        Returns a dict with evaluation metrics (mae, rmse, r2).
        """
    ...
    def predict(self, features: dict[str, Any]) -> dict[str, Any]:
        ...
        If the model has been trained on price history, uses the ensemble.
        Otherwise falls back to a heuristic estimate.
        ...
        # ── Ensemble prediction ──
        if self._is_trained and self.xgb_model is not None and self.rf_model is not None:
            # The fundamental model uses the 11 features; if trained on
            # TECHNICAL_FEATURES the shape differs, so we fall back to
            # a heuristic weighted sum for fundamental mode.
            predicted_price = self._predict_from_fundamentals(features)
            confidence_score = self._compute_confidence(features)
        else:
            # Untrained fallback – weighted heuristic
            predicted_price = self._predict_from_fundamentals(features)
            confidence_score = self._compute_confidence(features)
        ...
        # Feature importance – from XGBoost if trained on fundamentals, else heuristic
    ...
    def predict_price(
        ...
        """
        Predict future stock price from a single row of technical/OHLCV
        features (the same columns used during training).
        """
        ...
        return {
            ...
            "confidence_score": 0.7,  # placeholder; can be computed from prediction intervals
            "model": "xgboost_rf_ensemble",
            "version": self.MODEL_VERSION,
        }
    ...
    def _predict_from_fundamentals(self, features: dict[str, Any]) -> float:
        """
        Heuristic weighted score from the 11 fundamental features.

        This is used when the model hasn't been trained on fundamental
        → price mappings yet.  Replace with a properly trained model
        once you have labelled fundamental data.
        """
        weights = {
            "pe_ratio": -0.5,
            "debt_to_equity": -0.4,
